# -*- coding: utf-8 -*-
"""YouTube記事(post_009)用：ロング動画が広がる仕組み図を生成。
投稿直後は登録者→反応が良ければ関連動画・おすすめ欄に乗り一般視聴者へ拡散、を可視化。
ショート版(make_youtube_short.py)と対になるデザイン。
...
for i, (x, head, body) in enumerate(stages):
    ...
    ax.text(x, y-0.30, body, ha="center", va="center", fontproperties=fp(10.5, "bold"), color=tcols[i])
    if i < 3:
        ax.annotate("", xy=(stages[i+1][0]-bw/2-0.05, y), xytext=(x+bw/2+0.05, y),
                    arrowprops=dict(arrowstyle="-|>", color=BLUE, lw=2.4))
        ax.text((x+stages[i+1][0])/2, y+0.5, "視聴時間\n（長く見られたか）", ha="center", va="center",
                fontproperties=fp(8.0), color=BLUE)

# 初動が弱いと止まる（赤の注意）
ax.annotate("", xy=(2.9, 1.15), xytext=(1.5, y-bh/2-0.05),
            arrowprops=dict(arrowstyle="-|>", color=RED, lw=2, linestyle=(0,(4,2))))
ax.add_patch(FancyBboxPatch((3.0, 0.55), 6.4, 1.15, boxstyle="round,pad=0.02,rounding_size=0.1",
             fc="#fff0f0", ec="#f3c2c2", lw=1.2))
ax.text(6.2, 1.12, "初動（登録者の反応）が弱い・途中で離脱されると…",
        ha="center", va="center", fontproperties=fp(10.5, "bold"), color=RED)
ax.text(6.2, 0.8, "関連動画・おすすめ欄に乗らず、一般視聴者まで広がらない",
        ha="center", va="center", fontproperties=fp(9.5), color="#a05a5a")

ax.set_title("YouTubeロングが広がる仕組み（おすすめ・関連動画）", fontproperties=fp(15, "bold"), loc="left", x=0.01, pad=12)
fig.text(0.012, 0.02, "※判断材料は“再生数”より「視聴時間（どれだけ長く見られたか）」。初動の反応が次の拡散を呼ぶ。",
         fontproperties=fp(8.5), color="#9aa0a6")
fig.subplots_adjust(left=0.02, right=0.98, top=0.86, bottom=0.10)
fig.savefig(os.path.join(IMG, "youtube-longflow.png"), facecolor="white", bbox_inches="tight"); plt.close(fig)
print("saved youtube-longflow.png")
